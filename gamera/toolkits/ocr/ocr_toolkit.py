...
def __char_touches_top(glyph, line):
  """Returns true when the top of the character is close to the top of the
line."""
  #if glyph.ul_y < line.bbox.center_y-(line.bbox.nrows/4):
  if glyph.ul_y <= line.bbox.ul_y+(line.bbox.nrows/5):
      return True
  else:
      return False
...
"""
  ...
  for ccs in sub_ccs:
    ...
    add = []
    result = []
    glyphs.sort(key=lambda x: x.ul_x)
